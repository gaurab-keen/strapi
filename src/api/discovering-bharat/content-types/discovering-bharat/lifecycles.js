...
const {pathDiscover} = require('../../../commonFile/modelPathList');
...
module.exports = {
     ...
     beforeUpdate: async ({params})=>{
      const data=await getReviewDetailsById(pathDiscover,params.where.id); 
        if(data==null)
          return;
           ...
           if(params.data.publishedAt!=null || params.data.publishedAt!=undefined){
            strapi.log.debug("Data is Unpublished "+JSON.stringify(data.review))
              ...
              //  Unpublished button event
                strapi.log.debug("Data is Unpublished "+JSON.stringify(data.review))
                updateReviewById(pathDiscover,params.where.id,'Draft') //update data With Draft Mode When Unplished data
